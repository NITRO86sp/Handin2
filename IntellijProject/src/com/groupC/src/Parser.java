...
public class Parser {
    ...
    public  Job[] ParseInput(String[] file) {
        ...
        Job[] jobs = new Job[Integer.parseInt(fileLines[0].trim())];
        ...
        for (int i=2; i < fileLines.length; i++) {
            ...
            jobs[i-2] = new Job(Integer.parseInt(times[0].trim()), Integer.parseInt(times[1].trim()), i-2);
        }
    ...
    public  Output ParseOutput(String[] file) {
        ...
        for (int i=2; i < fileLines.length; i++) {
            String[] times = fileLines[i].split("\\s+");
            jobs[i-2] = new Job(Integer.parseInt(times[0].trim()), Integer.parseInt(times[1].trim()), i-2, Integer.parseInt(times[2].trim()));
        }
